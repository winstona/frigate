        ...
        while not self.stop_event.wait(wait_time):
            ...
            while True:
                try:
                    ...
                    if q_size > 5:
                        logger.warning(f"object_recordings_info loop queue not empty ({q_size}) - recording segments may be missing")
                    break

            if stale_frame_count > 0:
                logger.error(f"Found {stale_frame_count} old frames, segments from recordings may be missing")
            ...
            if self.audio_recordings_info_queue:
                ...
                while True:
                    try:
                        ...
                        if q_size > 5:
                            logger.warning(f"object_recordings_info loop audio queue not empty ({q_size}) - recording segments may be missing")
                        break

                if stale_frame_count > 0:
                    logger.error(f"Found {stale_frame_count} old audio frames, segments from recordings may be missing")
